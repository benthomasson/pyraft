...
class ArcusMonitor(raft.RaftNode):
    ...
    def do_failover(self, addr):
        ...
        if self.failover_count > self.failover_count_limit:  # start cooldown
            self.log_info(
                "failover count exceeds limit %d. start cooldown time"
                % self.failover_count_limit
            )
        ...
        try:
            ...
            self.log_info(
                "node %s is already deleted (maybe gracefully shutdown): %s"
                % (addr, str(e))
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-threshold",
        dest="failover_threshold",
        default=5,
        help="failover threshold (default 5)",
    )
    parser.add_argument(
        "-limit",
