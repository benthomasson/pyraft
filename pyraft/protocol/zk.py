...
class ZkConnect:
    ...
    def deserialize(self, buff, offset):
        ...
        (
            ...
            self.timeout,
            self.session_id,
        ) = int_long_int_long_struct.unpack_from(buff, offset)
    ...
    def serialize(self, b):
        b.extend(
            int_int_long_struct.pack(self.proto_ver, self.timeout, self.session_id)
        )
...
class zk_io(base_io):
    ...
    def relay_cmd(self, addr, cmd):  # cmd: ['create', ZkCreate]
        ...
        if addr not in self.relay_map:
            ...
            b.extend(
                int_long_int_long_struct.pack(-1, 0, 10000, cmd.session_id)
            )  # proto, last zxid, timeout, session_id
            s.send(b)
            ret = s.recv(4096)
