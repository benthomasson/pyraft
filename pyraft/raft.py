...
class RaftNode(object):
    def __init__(
        self, nid, addr, ensemble={}, peer=False, worker=None, overwrite_peer=False
    ):
        ...
        if worker is None:
            worker = MergedWorker(
                self.addr, BaseWorker(self.addr), RedisWorker(self.addr)
            )
        ...
        if "e" in handler[1]:
            ...
            if self.state != "l":
                for nid, p in self.get_peers().items():
                    if p.state == "l":
                        return self.worker_map[worker_offset].relay_cmd(
                            p, cmd, worker_offset
                        )
    ...
    def apply_loop(self):
        ...
        while True:
            ...
            if self.index >= item.index:
                self.log_info(
                    'skip log [%d:%d]: "%s"' % (self.index, item.index, str(cmd))
                )
                continue
    ...
    def process_raft_accept(self, sock):
        ...
        if nid != None:  # new peer
            ...
            if peer.addr != addr:
                ...
                if (
                    self.overwrite_peer
                ):  # delete previous nid automatically (usually used in k8s environment. pod restart)
                    self.del_node(nid)
    ...
    def select_peer_req(self, timeout):
        ...
        reads, writes, excepts = select.select(
            list(sock_peer_map.keys()), [], [], timeout
        )
        peers = []
        for r in reads:
    ...
    def handle_request(self, p, toks):
        ...
        if (
            toks[0] == "append_entry"
        ):  # append_entry, term, prev_term, prev_index, commit_index, ts, worker_offset, cmds...
            ts = toks[5]
            if len(toks) > 6:
            ...
            toks[0] == "snapshot"
        ):  # snapshot, term, prev_term, prev_index, commit_index, data
        ...
        while True:
            ...
            for toks in msg_list:
                ...
                if toks[0] == "ack":
                    ...
                    self.log_info(
                        'unknown append_entry resp. from %s: "%s"' % (p.nid, toks)
                    )
    ...
    def do_follower(self):
        ...
        if (
            ...
            and int(time.time()) - self.last_append_entry_ts > CONF_PING_TIMEOUT
        ):
            self.on_candidate()
            self.state = "c"
    ...
    def append_entry(self, future):
        ...
        if future != None:
            ...
            if n_ack > half:
                ...
                item = LogItem(
                    self.term, prev_index + 1, ts, future.worker_offset, future
                )
                self.log.push(item, self.commit_index)
    ...
    def do_leader(self):
        ...
        for nid, p in self.get_peers().items():
            ...
            if (
                now - p.last_delayed_ts > 2.0
                and p.raft_req.connected()
                and p.index < self.index
            ):
                p.last_delayed_ts = now
    ...
    def process_install_snapshot(self, p):
        ...
        if p.index < self.log.start_index() or (
            diff >= 100 or diff > len(self.data) / 10
        ):
...
def parse_default_args(parser):
    parser.add_argument(
        "-a",
        dest="addr",
        help="ip:port[port+1], :port means pick one ip by gethostbyname (ex. -a 127.0.0.1:5010)",
    )
    parser.add_argument(
        "-e",
        dest="ensemble",
        help="ensemble ip list or domain name with port (ex. -e 2/127.0.0.1:5020,3/127.0.0.1:5030 or -e 127.0.0.1:5020,127.0.0.1:5030 or -e pyraft.test.com:5010)",
    )
    parser.add_argument(
        "-i",
        dest="nid",
        help="self node id (if not exists, use address, HOSTNAME use machine hostname) (ex. -i 1)",
    )
    parser.add_argument("-l", dest="load", help="checkpoint filename to load")
    parser.add_argument(
        "-o",
        dest="overwrite_peer",
        help="overwrite duplicated nid node (delete previous one)",
        action="store_true",
    )
    parser.add_argument(
        "-loglevel",
        dest="loglevel",
        default="info",
        help="loglevel (debug, info, warning, error, fatal)",
    )
    ...
    if args.logfile is not None:
        handler = RotatingFileHandler(
            args.logfile, maxBytes=1024 * 1024, backupCount=10
        )
...
def make_default_node():  # redis interface node is default now
    ...
    node = RaftNode(
        args.nid, args.addr, args.ensemble_map, overwrite_peer=args.overwrite_peer
    )
